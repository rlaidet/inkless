...
class BatchBufferTest {
    ...
    void multipleTopicPartitions() {
        ...
        buffer.addBatch(T0P0, t0p0b0);
        buffer.addBatch(T1P0, t1p0b0);
        buffer.addBatch(T0P0, t0p0b1);
        buffer.addBatch(T1P0, t1p0b1);
        buffer.addBatch(T0P1, t0p1b0);
        buffer.addBatch(T0P0, t0p0b2);
        buffer.addBatch(T0P1, t0p1b1);
        buffer.addBatch(T0P1, t0p1b2);
        buffer.addBatch(T1P0, t1p0b2);
        ...
        final ByteBuffer expectedBytes = ByteBuffer.allocate(
            t0p0b0.sizeInBytes() + t0p0b2.sizeInBytes() + t0p0b1.sizeInBytes()
            + t0p1b0.sizeInBytes() + t0p1b2.sizeInBytes() + t0p1b1.sizeInBytes()
            + t1p0b0.sizeInBytes() + t1p0b2.sizeInBytes() + t1p0b1.sizeInBytes()
        );
    ...
    @Test
    void worksAfterClosing() {
        final BatchBuffer buffer = new BatchBuffer();

        final RecordBatch batch1 = createBatch(T0P0 + "-0");
        buffer.addBatch(T0P0, batch1);
        ...
        buffer.addBatch(T1P0, batch2);
        final BatchBuffer.CloseResult result2 = buffer.close();
        assertThat(result2.commitBatchRequests()).containsExactly(
            new CommitBatchRequest(T1P0, 0, batch2.sizeInBytes(), 1)
        );
        assertThat(result2.data()).containsExactly(batchToBytes(batch2));
    }
