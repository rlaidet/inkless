...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long,
    ...
    val (inklessEntries, classicEntries) = entriesPerPartition.partition { case (k, _) => _inklessMetadataView.isInklessTopic(k.topic()) }
    ...
    val inklessResponsesFuture = inklessAppendHandler match {
      ...
      case _ =>
        ...
        CompletableFuture.completedFuture(util.Map.of[TopicPartition, PartitionResponse]())
    }
  ...
  def fetchOffset(topics: Seq[ListOffsetsTopic],
    ...
    if (delayedRemoteListOffsetsRequired(statusByPartition)) {
      ...
      val delayedRemoteListOffsets = new DelayedRemoteListOffsets(delayMs, version, statusByPartition, this, _inklessMetadataView.isInklessTopic, responseCallback)
  ...
  def fetchParamsWithNewMaxBytes(originalParams: FetchParams, percentage: Float): FetchParams = {
    new FetchParams(originalParams.requestVersion, originalParams.replicaId, originalParams.replicaEpoch, originalParams.maxWaitMs, originalParams.minBytes,
        ...
        val inklessFetchResults = try {
          ...
          response.map { case (tp, data) =>
            ...
            tp -> new LogReadResult(
              ...
              data.divergingEpoch.toScala, data.highWatermark, data.logStartOffset, data.highWatermark, data.logStartOffset,
              0L, // fetchTimeMs is ignored
              if (data.lastStableOffset.isPresent) Some(data.lastStableOffset.getAsLong) else None,
              if (data.preferredReadReplica.isPresent) Some(data.preferredReadReplica.getAsInt) else None,
              exception.toScala
            )
          }
        } catch {
  ...
  private def maybeShrinkIsr(): Unit = {
    ...
    allPartitions.keys.foreach { topicPartition =>
      if (!_inklessMetadataView.isInklessTopic(topicPartition.topic()))
  ...
  def lastOffsetForLeaderEpoch(
    ...
    requestedEpochInfo
      .map { offsetForLeaderTopic =>
        if (_inklessMetadataView.isInklessTopic(offsetForLeaderTopic.topic())) {
  ...
  private def applyLocalLeadersDelta(
    ...
    localLeaders.foreachEntry { (tp, info) =>
      if (!_inklessMetadataView.isInklessTopic(tp.topic()))
